...
alpha0 = -atan(Y/X)+acos((X^2+Y^2+L^2-(rr+rb)^2)/(2*L*sqrt(X^2+Y^2)));
%%%%follower angle%%%%
for i = 1:1:length(Theta)
    if Theta(i)<beta
        %gate angle
        alpha(i) = alpha0 + flift(i);
        %contact angle on wheel
        Chi (i) = atan((Y+L*(1+fliftDot(i))*sin(alpha(i)))/(X-L*(1+fliftDot(i))*cos(alpha(i))));
    elseif Theta(i)>(2*pi-beta)
        alpha(i) = alpha0 + freturn(i);
        %contact angle on wheel
        Chi (i)= atan((Y+L*(1+freturnDot(i))*sin(alpha(i)))/(X-L*(1+freturnDot(i))*cos(alpha(i))));
    else
        alpha(i) = alpha0 + delAlpha;
        %contact angle on wheel
        Chi (i)= atan((Y+L*sin(alpha(i)))/(X-L*cos(alpha(i))));
    end
end
...
for i = 1:1:length(Theta)
    ...
    plot (Xp*cos(Theta(i)+31*pi/180+pi/2)-Yp*sin(Theta(i)+31*pi/180+pi/2),...
    Xp*sin(Theta(i)+31*pi/180+pi/2)+Yp*cos(Theta(i)+31*pi/180+pi/2),'Color',[0, 0.4470, 0.7410],'LineWidth', 3,...
    'DisplayName', 'CAM') 
...
plot (Theta*180/pi,alpha*180/pi, 'DisplayName', 'Gate angle, °')
ylabel('α, °')
xlabel('θ, °')
hold off
